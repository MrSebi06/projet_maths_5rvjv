...
const float LiquidBody::NEIGHBORHOOD_RADIUS = 0.1f;
const float LiquidBody::REST_DENSITY = 75;
...
LiquidBody::LiquidBody(Transform *transform, const float &mass, const float &restitution, float friction,
                       CollisionShape *shape, bool debug) : Rigidbody2D(transform, mass, restitution, friction, shape, debug)
{
...
void LiquidBody::integrate(float dt)
{
    ...
    for (auto neighbor : neighbors)
    {
        ...
        // Add Pressure force
        auto pressure_strength = -neighbor->mass * (pressure + neighbor->pressure) / (2 * neighbor->density) * pressure_kernel(r, NEIGHBORHOOD_RADIUS);
